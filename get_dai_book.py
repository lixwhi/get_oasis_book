...
outfilename = 'testing06.png'
start_block_num = 7999000
end_block_num = 8106000
...
TX_ID_TOPIC_ADDRESS = '0xa2c251311b1a7a475913900a2a73dc9789a21b04bc737e050bbc506dd4eb3488'
TX_ID_LOGSORTEDOFFER = '0x20fb9bad86c18f7e22e8065258790d9416a7d2df8ff05f80f82c46d38b925acd'
TX_ID_LOGMAKE = '0x773ff502687307abfa024ac9f62f9752a0d210dac2ffd9a29e38e12e2ea82c82'
TX_ID_LOGTAKE = '0x3383e3357c77fd2e3a4b30deea81179bc70a795d053d14d5b7f2f01d0fd4596f'
TX_ID_LOGKILL = '0x9577941d28fff863bfbee4694a6a4a56fb09e169619189d2eaa750b5b4819995'
		...
		if ((len(logmakes_pre) != 0) or (len(logkills_pre) != 0) or (len(logtakes_pre) != 0)):
			...
			for j in range(0, len(logtakes_pre)):
				...
				if((paygem == WETH0X) and (buygem == DAI0X)):
					...
					amtdai = payamt
					# print('buy')
					# print('give_amt {0}'.format(payamt))
					# print('take_amt {0}'.format(buyamt))
				if((paygem == DAI0X) and (buygem == WETH0X)):
					boa = -1
					amtdai = buyamt
					# print('sell')
...
def remove_kills(logkills, ob):
	indx = []
	#print('want to remove {0}'.format(logkills))
	for w in logkills:
		#print('need to remove: {0}'.format(w))
	...
	if(len(indx) != 0):
		...
		for w in indx:
			...
			rem_count += 1
			#print('removed id: {0}'.format(ss.oid))
	...
	for w in logtakes:
		for p in range(0, len(ob.book)):
			if(w.oid == ob.book[p].oid):
				...
				#print('take id: {0}, dai left: {1}'.format(ob.book[p].oid, ob.book[p].dai_remaining))
	return ob
...
def remove_dust(ob):
	...
	if(len(indx) != 0):
		...
		for w in indx:
			ss = ob.book.pop(w - rem_count)
			rem_count += 1
			#print('removed dust id: {0}'.format(ss.oid))
